...
function set_lang_id($lang_id) {
    if (strlen($lang_id) == 2 && is_language_supported($lang_id)) {
        $language_file = includeTrailingCharacter(realpath(dirname(__FILE__)), DIRECTORY_SEPARATOR) . $lang_id . "_language.php";
...
function get_lang_id() {
    return ( isset($_COOKIE['lang']) && strlen($_COOKIE['lang']) == 2 && is_language_supported($_COOKIE['lang']) ) ? htmlspecialchars($_COOKIE['lang']) : 'sr';
}
